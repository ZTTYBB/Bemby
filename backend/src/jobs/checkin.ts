    ...
    for (const row of allBtnRows) {
      for (const btn of row.buttons) {
        ...
        if (matches) {
          ...
          // Some bots never answer the callback query (they act via a follow-up
          // message / Cloudflare page instead), so Telegram returns
          // BOT_RESPONSE_TIMEOUT. Treat that as "click sent" and read the follow-up;
          // the listeners above are already running so no response is missed.
          let answer: Api.messages.BotCallbackAnswer | undefined;
          try {
