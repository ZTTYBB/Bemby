...
const zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      inAppButtonPlaceholder: "例：delay(10000) > scroll(0, 800) > {aiBtn} > 确认签到",
      inAppButtonHint: "打开小程序页面后按顺序执行，用 > 或 -> 分隔。可用步骤：①按钮文字，如 立即签到（优先匹配真正的按钮/链接，而不是同名的说明文字）；②css: 选择器，如 css:#checkin-btn；③delay(10000) 等待 10 秒再执行下一步（也可写 delay(10s)），适用于页面加载较慢的小程序；④scroll(0, 800) 按像素滚动页面（第一个数字为横向 x，第二个为纵向 y；只写一个数字表示纵向，如 scroll(800)；也可写 scroll(y=800)、scroll(x=-200)；负数为反向滚动），用于把屏幕外的按钮滚到可见区域，数值超过页面长度时直接滚到尽头，例如 scroll(0, 99999) 滚到底部；⑤{aiBtn} 截图后由 AI 判断该点哪个按钮，可用 {aiBtn:签到按钮} 补充说明；⑥{input} 自动计算算术验证码并填入；⑦{aiInput} 由 AI 读题并填入。常用组合：delay(10000) > {aiBtn}（等页面加载完再让 AI 找按钮）；scroll(0, 99999) > 立即签到（先滚到底部再点按钮）。留空则自动识别签到类按钮。按钮已置灰或显示已完成时不会点击；若只点到普通文字且页面没有反应，该操作会报错而不是误报成功",
...
const en: typeof zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      inAppButtonPlaceholder: "e.g. delay(10000) > scroll(0, 800) > {aiBtn} > Confirm",
