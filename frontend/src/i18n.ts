...
const zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      inAppButtonPlaceholder: "例：立即签到 > {aiInput} > 确认签到",
      inAppButtonHint: "打开小程序页面后按顺序执行，用 > 分隔：填写按钮文字则点击该按钮（优先匹配真正的按钮/链接，而不是同名的说明文字）；以 css: 开头可直接指定 CSS 选择器，例如 css:#checkin-btn；{input} 自动计算算术验证码并填入输入框；{aiInput} 由 AI 读题并填入。留空则自动识别签到类按钮。按钮已置灰或显示已完成时不会点击；若只点到普通文字且页面没有反应，该操作会报错而不是误报成功",
  ...
  settings: {
    ...
    cfSolver: {
      ...
      hint: "部分站点签到需要通过 Cloudflare 的“我不是机器人”验证。启用后会按需下载一个浏览器到数据目录（约 150MB），镜像本身保持精简。需要容器可访问网络。",
      status: "状态",
      stateInstalled: "已安装",
      ...
      fontsMissing: "缺少中日韩 / Emoji 字体，浏览器目前只能显示拉丁字符，可能降低验证通过率。点击上方安装按钮即可补齐（约 30MB，无需重新下载浏览器）。",
      fontsFailed: "字体下载失败，浏览器仍可使用（仅拉丁字符）。请检查网络后重新安装。",
      reinstallBtn: "重新下载 / 更新浏览器",
      reinstallHint: "重新下载会覆盖现有浏览器，用于升级版本或替换旧版本安装的构建（约 170MB）。",
      testBtn: "浏览器自检",
      testing: "正在检测…",
...
const en: typeof zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      inAppButtonPlaceholder: "e.g. Check in > {aiInput} > Confirm",
      inAppButtonHint: "Run in order once the Mini App page is up, separated by >: text presses the control carrying that label, preferring a real button or link over a caption that happens to read the same; a step starting with css: is a CSS selector, e.g. css:#checkin-btn; {input} solves an arithmetic captcha and types it; {aiInput} lets the AI read the question and answer it. Blank auto-detects a checkin-worded control. A disabled or already-completed control is left alone, and if only plain text could be pressed and the app did not react, the action fails rather than reporting a checkin that never happened",
  ...
  settings: {
    ...
    cfSolver: {
      ...
      hint: "Some sites gate checkin behind Cloudflare's \"I am not a bot\" check. Enabling this downloads a browser on demand into your data dir (~150MB); the image itself stays small. Requires the container to have internet access.",
      status: "Status",
      ...
      fontsMissing: "CJK / emoji fonts are missing, so the browser can only draw Latin text, which may lower the pass rate. Use the install button above to fetch them (~30MB, the browser is not re-downloaded).",
      ...
      reinstallHint: "Downloads over the existing browser (~170MB), to update it or to replace a build left by an earlier version.",
