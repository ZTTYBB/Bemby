...
const zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      inAppButtonHint: "打开小程序页面后按顺序执行，用 > 分隔：填写按钮文字则点击该按钮；{input} 自动计算算术验证码并填入输入框；{aiInput} 由 AI 读题并填入。留空则自动识别签到类按钮。按钮已置灰或显示已完成时不会点击",
      labelMiniAppMaxWait: "浏览器最长等待（毫秒）",
      miniAppMaxWaitHint: "浏览器打开小程序的总时长上限（包含所有代理尝试）。留空或 0 使用默认 300000（5 分钟），超时后该操作失败",
      ...
      miniAppTryAllHint: "验证被拒绝时依次尝试代理列表中的其他代理（每次重新签名小程序链接）；关闭则只使用上面选定的代理",
    },
  },
  templates: {
    title: "模板",
...
const en: typeof zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      inAppButtonHint: "Run in order once the Mini App page is up, separated by >: plain text presses the control with that label; {input} solves an arithmetic captcha and types it; {aiInput} lets the AI read the question and answer it. Blank auto-detects a checkin-worded control. A disabled or already-completed control is left alone",
      labelMiniAppMaxWait: "Browser max wait (ms)",
      miniAppMaxWaitHint: "Budget for the whole browser part, including every proxy tried. Blank or 0 uses the default of 300000 (5 minutes); the action fails once it runs out",
      ...
      miniAppTryAllHint: "Works through the rest of the proxy list when an exit is refused, re-signing the Mini App link each time. Off keeps to the proxy chosen above",
    },
  },
  templates: {
    title: "Templates",
