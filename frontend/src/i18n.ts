...
const zh = {
  ...
  accounts: {
    ...
    bulkAdd: {
      btn: "批量添加账户",
      title: "批量添加账户",
      hint: "每行一个账户，格式为 手机号----API地址。系统会先创建全部账户，然后逐个自动认证（请求验证码、从 API 页面读取验证码与二步验证密码、等待 60 秒后继续下一个）。",
      start: "开始",
...
const en: typeof zh = {
  ...
  accounts: {
    ...
    bulkAdd: {
      ...
      hint: "One account per line, formatted as phone----apiUrl. All accounts are created first, then authenticated one by one (request code, read the code and 2FA from the API page, wait 60s, then continue).",
      start: "Start",
