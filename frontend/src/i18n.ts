...
const zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      inAppButtonPlaceholder: "例：delay(10000) > {aiBtn} > 确认签到",
      inAppButtonHint: "打开小程序页面后按顺序执行，用 > 或 -> 分隔。可用步骤：①按钮文字，如 立即签到（优先匹配真正的按钮/链接，而不是同名的说明文字）；②css: 选择器，如 css:#checkin-btn；③delay(10000) 等待 10 秒再执行下一步（也可写 delay(10s)），适用于页面加载较慢的小程序；④{aiBtn} 截图后由 AI 判断该点哪个按钮，可用 {aiBtn:签到按钮} 补充说明；⑤{input} 自动计算算术验证码并填入；⑥{aiInput} 由 AI 读题并填入。常用组合：delay(10000) > {aiBtn}（等页面加载完再让 AI 找按钮）；立即签到 > {input} > 确认。留空则自动识别签到类按钮。按钮已置灰或显示已完成时不会点击；若只点到普通文字且页面没有反应，该操作会报错而不是误报成功",
  ...
  settings: {
    ...
    cfSolver: {
      title: "Cloudflare 验证（我不是机器人）",
      hint: "部分站点签到需要通过 Cloudflare 的“我不是机器人”验证。启用后会按需下载 CloakBrowser（一个指纹经源码级修补的 Chromium）到数据目录（约 200MB），镜像本身保持精简。需要容器可访问网络。",
      status: "状态",
      stateInstalled: "已安装",
...
const en: typeof zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      inAppButtonPlaceholder: "e.g. delay(10000) > {aiBtn} > Confirm",
      inAppButtonHint: "Run in order once the Mini App page is up, separated by > or ->. Steps available: plain text presses the control carrying that label, preferring a real button or link over a caption that happens to read the same; css: is a CSS selector, e.g. css:#checkin-btn; delay(10000) waits 10 seconds before the next step (delay(10s) also works), for an app that is slow to settle; {aiBtn} screenshots the page and lets the AI pick the button to press, with {aiBtn:the checkin button} to say which one; {input} solves an arithmetic captcha and types it; {aiInput} lets the AI read the question and answer it. Common combinations: delay(10000) > {aiBtn} lets the page finish loading before the AI looks at it; Check in > {input} > Confirm. Blank auto-detects a checkin-worded control. A disabled or already-completed control is left alone, and if only plain text could be pressed and the app did not react, the action fails rather than reporting a checkin that never happened",
  ...
  settings: {
    ...
    cfSolver: {
      title: "Cloudflare solving (I am not a bot)",
      hint: "Some sites gate checkin behind Cloudflare's \"I am not a bot\" check. Enabling this downloads CloakBrowser -- a Chromium whose fingerprint is patched at source -- on demand into your data dir (~200MB); the image itself stays small. Requires the container to have internet access.",
      status: "Status",
