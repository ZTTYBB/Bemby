...
const zh = {
  ...
  jobs: {
    ...
    web: {
      ...
      labelOwnProfile: "为本任务单独保存 Cookie",
      ownProfileHint:
        "浏览器的 Cookie 保存在「配置文件」中，默认按出口（代理）共享——同一出口下的所有任务共用一份，多个账号会互相覆盖登录状态。勾选后本任务拥有独立配置文件，登录状态互不干扰，可跨运行保留（配合上面的条件分支即可只登录一次）。需要登录的任务请勾选；其他任务保持关闭，以便共享 cf_clearance 并减少磁盘上的配置文件数量。注意「设置」中的配置文件数量上限（默认 12，按最近使用淘汰），任务较多时请相应调高",
      labelTimes: "循环次数",
      timesHint:
...
const en: typeof zh = {
  ...
  jobs: {
    ...
    web: {
      ...
      labelOwnProfile: "Keep this job's cookies to itself",
      ownProfileHint:
        "Cookies live in the browser profile, which by default is shared by every job going out through the same exit -- so several accounts overwrite each other's session. On, this job gets a profile of its own: its login is private to it and survives to the next run, which together with a condition above means logging in once rather than every time. Turn it on for anything that logs in, and leave it off elsewhere so a cf_clearance is pooled and fewer profiles pile up on disk. Mind the profile limit in Settings (12 by default, least recently used dropped first) and raise it if you have many such jobs",
