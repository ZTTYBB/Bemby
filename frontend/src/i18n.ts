...
const zh = {
  ...
  accounts: {
    ...
    resendAnyway: "仍然尝试重发",
    codeNoDispatchHint:
      "本次响应既没有重发倒计时也没有备用方式，通常说明 Telegram 受理了请求但并未真正下发短信（号码或 api_id 被过滤）。可尝试重发看返回什么，或更换号码/代理/api_id。",
    codeDelivery: "Telegram 发送方式",
    codeNextType: "备用方式（重发时使用）",
    codeNextTypeNone: "无",
    codeRetryAfter: "{n} 秒后可重发",
    codeLength: "验证码长度：{n} 位",
    codeUndeliverable:
      "Telegram 选择了只有官方客户端才能完成的验证方式（设备完整性校验或应用内邮箱），因此不会收到验证码。请尝试重发以使用备用方式，或更换代理/号码后重试。",
    codeType: {
      app: "Telegram 应用内消息",
      sms: "短信",
      call: "语音电话（播报验证码）",
      flashcall: "闪呼（来电号码即验证码）",
      missedcall: "未接来电（号码后几位即验证码）",
      fragmentsms: "Fragment 匿名号码（在 Fragment 网站查看）",
      firebasesms: "短信（需通过设备完整性校验，第三方客户端无法完成）",
      emailcode: "邮箱验证码",
      setupemailrequired: "需先在官方客户端设置登录邮箱",
      smsword: "短信（一个单词）",
      smsphrase: "短信（一句短语）",
    },
    codeNoReceiveHint:
    ...
    authenticate: "认证",
    signUpHint:
      "该号码尚未注册 Telegram 账号。填写姓名即可用此号码注册新账号。",
    signUpTosNote:
      "注册即表示接受 Telegram 服务条款。新注册账号常被立即限制，建议注册后检测状态。",
    signUp: "注册账号",
    signingUp: "注册中...",
    ...
    errors: {
      ...
      flood: "操作过于频繁，请稍后再试",
      signUpNameInvalid: "姓名无效，请更换后重试",
      signUpNumberOccupied: "该号码已注册账号，请返回使用验证码登录",
      signUpRestricted: "Telegram 拒绝在此号码上注册（可能需要更换代理或号码）",
      apiCredsRequired:
...
const en: typeof zh = {
  ...
  accounts: {
    ...
    resendNext: "Resend using the fallback method",
    resendAnyway: "Try a resend anyway",
    codeNoDispatchHint:
      "This response carried neither a resend timeout nor a fallback method, which usually means Telegram accepted the request without actually sending an SMS (the number or api_id is filtered). Try a resend to see what it answers, or retry with a different number, proxy or api_id.",
    codeDelivery: "Telegram sent it via",
    codeNextType: "Fallback used on resend",
    codeNextTypeNone: "none offered",
    codeRetryAfter: "Resend allowed after {n}s",
    codeLength: "Code length: {n} digits",
    codeUndeliverable:
      "Telegram chose a method only the official app can complete (device integrity check or an in-app email), so no code will arrive. Try the resend to use the fallback method, or retry with a different proxy or number.",
    codeType: {
      app: "Telegram app message",
      sms: "SMS",
      call: "Voice call reading out the code",
      flashcall: "Flash call (the calling number is the code)",
      missedcall: "Missed call (last digits of the number are the code)",
      fragmentsms: "Fragment anonymous number (check the Fragment site)",
      firebasesms:
        "SMS gated behind a device integrity check that third-party clients cannot pass",
      emailcode: "Email code",
      setupemailrequired: "A login email must be set up in the official app first",
      smsword: "SMS containing a single word",
      smsphrase: "SMS containing a phrase",
    },
    codeNoReceiveHint:
    ...
    signUpHint:
      "This number has no Telegram account yet. Enter a name to register a new one on it.",
    signUpTosNote:
      "Registering accepts Telegram's Terms of Service. New accounts are often spam-limited straight away, so check the status afterwards.",
    ...
    errors: {
      ...
      signUpNameInvalid: "Telegram rejected that name, try a different one",
      signUpNumberOccupied:
        "This number already has an account -- close and log in with a code instead",
      signUpRestricted:
        "Telegram refused to register this number (a different proxy or number may be needed)",
      apiCredsRequired:
